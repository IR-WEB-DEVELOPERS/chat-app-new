...
async function loadFriendsList() {
    ...
    try {
        ...
        // Fetch last message for each friend to sort + preview
        const friendEntries = await Promise.all(friends.map(async (friendUID) => {
            ...
            try {
                ...
                if (!lastMsgSnap.empty) {
                    ...
                    // Time label: today → HH:MM, older → date
                    const now = new Date();
                    const isToday = tDate.toDateString() === now.toDateString();
                    const yesterday = new Date(now); yesterday.setDate(now.getDate() - 1);
                    const isYesterday = tDate.toDateString() === yesterday.toDateString();
                    if (isToday) lastTimeStr = tDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                    else if (isYesterday) lastTimeStr = 'Yesterday';
                    else lastTimeStr = tDate.toLocaleDateString('en-GB', { day: '2-digit', month: 'short' });
                    // Preview text
        ...
        // Sort: friends with latest message first
                ...
                <button class="chat-item" data-uid="${safeFriendUID}">
                    ...
                    <div class="chat-info">
                        <div class="chat-item-top">
                            <h4>${safeName}</h4>
                            ${lastTimeStr ? `<span class="chat-item-time">${lastTimeStr}</span>` : ''}
                        </div>
...
function setReply(msg, chatType) {
    replyingTo = { id: msg.id, text: msg.text || '', senderName: msg.senderName || 'User' };
...
function attachMessageActionListeners(container, messages, chatType) {
    container.querySelectorAll('.msg-action-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (action === 'reply') setReply(msg, chatType);
